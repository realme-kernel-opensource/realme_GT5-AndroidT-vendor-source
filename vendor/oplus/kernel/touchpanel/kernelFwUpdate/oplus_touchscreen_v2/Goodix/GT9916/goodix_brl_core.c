			...
			struct goodix_ts_cmd *cmd)
{
	...
	if (!misc->cmd_addr) {
		TPD_INFO("GT_brlD:%s: invalied cmd addr\n", __func__);
		return -1;
	}
	cmd->state = 0;
	cmd->ack = 0;
...
{
	TPD_INFO("GT_brlD:%s find vibrator cmd and set cmd:%d\n", __func__, BIT_CHK(ctl_cmd, PEN_CTL_VIBRATOR_ENABLE));
...
static void goodix_get_pen_points(void *chip_data, struct pen_info *pen_info)
{
	...
	TPD_DEBUG("GT_brlD:%s:pen data=[%*ph]\n", __func__,
		IRQ_EVENT_HEAD_LEN, chip_info->touch_data);
...
static void goodix_get_health_info(void *chip_data, struct monitor_data *mon_data)
{
	...
	if (chip_info->pen_enable) {
		...
		if (!pen_str) {
			TPD_INFO("GT_brlD:pen_str kzalloc failed.\n");
		} else {
...
static void init_gt9916_debug_node(struct chip_data_brl *chip_info)
{
...
static int goodix_gt9916_ts_probe(struct spi_device *spi)
{
	...
	init_gt9916_debug_node(chip_info);
...
ts_malloc_failed:
	...
	TPD_INFO("GT_brlD:%s, probe error\n", __func__);
	return ret;
}
